...
/* Function to take phraseString[1] and place it on puzzle.
 * may require shuffling up to 2 colulmns and 2 rows.
 */
void SudokuBoard::findHighLights() {
	// first step is to start with row 5.
	int  firstrowpointer = 9 * 5; // points to first number in 5th row?
	int secondrowpointer = 9 * 6; // points to first number in 6th row?
	...
	{	if ( i != secondletter )
			highLites[firstrowpointer+i] = 1 ;
		else
			highLites[secondrowpointer+i] = 1 ;
	}
	for ( int i = 0; i < 9; i++ )
	{
		lettermap[i] = solution[firstrowpointer+i]; // fill in letter map with row 5.
...
//	phraseString[0] = phraseString[1][lettermap[0]-1]; // reset 0 to 9 characters
	for ( int i = 0; i < 9; i++ )  // set up 0 to have letters in correct order.
		phraseString[0][lettermap[i]-1] = phraseString[1][i] ;

}

void SudokuBoard::moveRow(int row, int secondrowpointer)
{
	int temp ;
	row *= 9 ;
	...
	{	temp = solution[secondrowpointer+i] ;
		...
		solution[secondrowpointer+row+i] = temp ;
		temp = puzzle[secondrowpointer+i] ;
		puzzle[secondrowpointer+i] = puzzle[secondrowpointer+row+i] ;
		puzzle[secondrowpointer+row+i] = temp ;
	}
}
void SudokuBoard::moveCol(int col, int secondcolpointer)
{
	...
	{	temp = puzzle[secondcolpointer + (i*9)] ;
		puzzle[secondcolpointer + (i*9)] = puzzle[col + (i*9)] ;
		puzzle[col + (i*9)] = temp ;
		temp = solution[secondcolpointer + (i*9)] ;
		solution[secondcolpointer + (i*9)] = solution[col + (i*9)] ;
		solution[col + (i*9)] = temp ;
	}

}

/*
void SudokuBoard::fillUnused() {
	string blank = "         " ;
	int j = 0 ;
	for ( ; j < phraseString[0].length() ; j++ )
	{	if ( phraseString[0][j] == blank[j] )
			break ;
	}
	// increments j until blank space
	while ( phraseString[1][j] != " " )
			; // increments j until blank space

	int k = j ; // j is first space
	int m = 0 ; // index of unused characters
	while ( k < phraseString[0].length() )
	{	int i = 0 ;
		for ( ; i < j ; i++ )
		{	if ( touse[m] == phraseString[0][i] )
				break ;
		}
		if ( i == j ) // didn't find a match
			phraseString[0][k++] = touse[m++] ;
	}

}*/

void SudokuBoard::setPhrase(string* inphraseString){
	string blank = "         " ;
	string unused = "ZQXJKVBPG" ; // least used letters in English:  Z,Q,X,J,K,V,B,P,G
	phraseString = inphraseString;

	if (phraseString[0].length() > 0)
	{	phraseString[1] = phraseString[0] ; // we're finding first (and only) duplicate in string
									        // the second string holds the modified string.
/*		for ( int i = phraseString[0].length(); i < 9 ; i++)
			(phraseString[0])+=" "; // fill rest of string with blanks
*/
		int j ;
		for ( int i = 0 ; i < unused.length() ; i++ )
		{	for ( j = 0 ; j < phraseString[0].length(); j++ )
			{	if ( unused[i] == phraseString[0][j] )
					break ;
			}
			if ( j == phraseString[0].length() ) // we got to end, not used
				touse += unused[i];	// touse contains all extra letters to fill in string with
		}
		int k = 0 ;
		for ( int i = 0 ; i < phraseString[0].length() - 1; i++ ) // we only need to go to
																  // next to last letter.
		{	for ( j = i+1 ;
				( j < phraseString[0].length() && firstletter < 0 );
				  j++ ) // we can stop when duplicate found.
			{	if ( (phraseString[0][j] == phraseString[0][i]) )
				{	firstletter = i ;	// this is the position in the string of the first letter repeated,
					secondletter = j ;	// and this is where it's repeated.
					phraseString[1][j] = touse[k++] ; // put an unused character in the repeated character space.
					break ; // we've found the only allowed repeat
				}
			}
		}
...
/**
 * Print the wordoku puzzle.
 */
void SudokuBoard::printHighlights(){
    print(highLites);
}
